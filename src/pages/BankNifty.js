    ...
    await fetch(nse_api)
      ...
      .then((json) => {
        ...
        for (var i of json.data) {
          // console.log('demo',i);
          if (i.percentage.option === "BANKNIFTY CE" && i.status === "BUY" && i.call_put === "CALL") {
        ...
        for (var i of json.data) {
          ...
          if (i.percentage.option === "BANKNIFTY PE" && i.status === "BUY" && i.call_put === "PUT") {
        ...
        for (var j of json.data) {
          ...
          if (Buy_Date === currentDate && j.percentage.option === "BANKNIFTY CE") {
        ...
        for (var j of json.data) {
          var Buy_Date = moment(j.buy_time).format("D/MM/YYYY");
          if (Buy_Date === currentDate && j.percentage.option === "BANKNIFTY PE") {
            if (j.final_status === "PROFIT") {
              profit1 = profit1 + 1;
            } else if (j.final_status === "LOSS") {
              loss1 = loss1 + 1;
        ...
        json.data.map((val) => {
          ...
          // console.log(val);
          // SELL CONDITION
          if (val.status === "BUY" && val.percentage.option === "BANKNIFTY CE" && val.call_put === "CALL") {
          ...
          if (val.status === "BUY" && val.percentage.option === "BANKNIFTY PE" && val.call_put === "PUT") {
            let sell_Price = // val.sell_price;
              (val.buy_price * val.percentage.profit_percentage) / 100 +
              val.buy_price;
          ...
          if (buyConditionput === true) {
            buyputPrice.map((ab) => {
              if (put === "PUT") {
                ...
                let squareoff = (bdprice * profitPercentage) / 100;
                let stoploss = (bdprice * lossPercentage) / 100;
                let sell_price = (bdprice * profitPercentage_put) / 100 + bdprice;
      ...
      <div id='chartContainer'>
        <Table className='mt-3' id='chartContainer'>
          <thead>
            ...
            ><th width='8%' title='Open Interest in contracts'>
              ...
              return (
                <tr key={i}>
                  <td style={{
                    ...
                    {(data.PE.openInterest + data.PE.changeinOpenInterest) - (data.CE.openInterest + data.CE.changeinOpenInterest)}
                  </td>
                  <td
                    style={{
                  ...
                  >
                    {data.CE.openInterest + data.CE.changeinOpenInterest} ({((data.PE.changeinOpenInterest < 0) ? (data.PE.changeinOpenInterest) : null)})
                  </td>
                </tr>
              );
            })}
            ...
            {graterThanLive.slice(0, 10).map((data, i) => {
              return (
                <tr key={i}>
                  ...
                  >
                    {data.PE.changeinOpenInterest + data.PE.openInterest} ({((data.PE.changeinOpenInterest < 0) ? (data.PE.changeinOpenInterest) : null)})
                  </td>
